...
function sample(f::LowRankFun{Chebyshev,Chebyshev,TensorSpace{(Chebyshev,Chebyshev),Float64,2},Float64,Float64},n::Integer)
    ry=sample(sum(f,1),n)
    fA=evaluate(f.A,ry)
    CB=coefficients(f.B)
    AB=CB*fA
...
function sample{SS}(f::LowRankFun{LineSpace{SS},LineSpace{SS},TensorSpace{(LineSpace{SS},LineSpace{SS}),Float64,2},Float64,Float64},n::Integer)
